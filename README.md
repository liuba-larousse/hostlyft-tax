...
## The two spreadsheets — which is which

There are **two** Google Sheets. They have similar names and opposite rules,
and confusing them is the one mistake this system has actually made.

| | **Hostlyft_Accounting_2026** | **Hostlyft_Tax_2026** |
|---|---|---|
| Whose | **Yours.** Hand-maintained, full of your formulas | Generated by this tool |
| In `tax/.env` | `GOOGLE_SHEET_ID` | `GOOGLE_TAX_SHEET_ID` |
| This tool | **only ever READS it** | rewrites it on every run |
| Contains | your monthly tabs and split calculations | Summary, Income, Expenses, Excluded, Jars, Review, Reconciliation, Distributions, Checks |
| Safe to overwrite | **never** | yes, that is the point |

**Generated tabs live in the TAX sheet.** If you go looking for `Distributions`
or `Reconciliation` in your accounting sheet, they are not there and never will
be — that is by design, not a missing step.

To see both at once, with their roles printed beside them:

```
python scripts/check_google.py
```

**Why this has its own section.** On 12 September 2026 a review opened the
accounting sheet looking for the generated tabs, found only your original 16,
and reported that Stage 11 had never been done. It had been — the tabs were in
the other sheet the whole time. Nothing was damaged, because the write path
never pointed at your sheet; the mistake was in the *reading*. `GOOGLE_SHEET_ID`
reads like "the sheet" when it is the single place writing is forbidden.

Now a write aimed at your accounting sheet **raises an error instead of
writing**, `check_google.py` always shows both sheets side by side, and tests
pin all of it.
...
The tax sheet, which this system does own, is separate. See below — knowing
which is which matters more than it sounds.
